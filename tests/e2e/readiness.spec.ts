...
test("homepage start trial CTA opens a valid trial route", async ({ page }) => {
  ...
  const startTrial = page.getByRole("link", { name: /start (the )?14-day guided trial/i }).first();
...
test("homepage command center tabs and recovery flow are interactive", async ({ page }) => {
  await page.context().clearCookies();
  await page.setViewportSize({ width: 1440, height: 960 });
  await page.goto("/", { waitUntil: "domcontentloaded" });

  await expect(page.locator("#future-hero-title")).toContainText(/missed messages/i);

  const revenueTab = page.getByRole("tab", { name: "Revenue" });
  await expect(revenueTab).toBeVisible();
  await revenueTab.click();
  await expect(page.getByRole("tabpanel")).toContainText(/Money-at-risk scanner/i);

  const aiTab = page.getByRole("tab", { name: "AI" });
  await aiTab.click();
  await expect(page.getByRole("tabpanel")).toContainText(/Human-reviewed AI assist/i);

  const flowStep = page.getByRole("button", { name: /AI drafts the reply/i });
  await flowStep.click();
  await expect(flowStep).toHaveAttribute("aria-pressed", "true");
...
test("landing logo stays compact and light theme keeps dental backdrop balanced", async ({
  ...
  const toothOpacity = await page
    ...
    .first()
    .evaluate((element) => {
      const imageOpacity = Number(window.getComputedStyle(element).opacity);
      const parentOpacity = element.parentElement
        ? Number(window.getComputedStyle(element.parentElement).opacity)
        : 1;

      return imageOpacity * parentOpacity;
    });
...
test("public trust, health, and fallback pages render safely", async ({ page }) => {
  ...
  await expect(page.locator("body")).toContainText(/Dash Dental|Dental Recovery/i);
  await expect(page.locator("body")).toContainText(/page not found|does not exist/i);
});
...
test("workspace shell uses compact AI Studio layout and Ask AI opens cleanly", async ({
  ...
  const themeToggle = page.getByRole("button", { name: /Switch theme/i }).first();
